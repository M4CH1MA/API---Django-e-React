...
class Authentication:
    def signin(self, email=None, password=None) -> User:
        exception_auth = AuthenticationFailed("Email ou senha nao encontrado")
    ...
    def signup(self, name, email, password, type_account='owner', company_id=False):
        if not name or name == '':
            raise APIException('O nome nao pode ser vazio')
        
        if not email or email == '':
            raise APIException('O email nao pode ser vazio')
        
        if not password or password == '':
            raise APIException('O password nao pode ser vazio')
        ...
        if user.objects.filter(email=email).exists():
            raise APIException('Este e-mail ja existe')
